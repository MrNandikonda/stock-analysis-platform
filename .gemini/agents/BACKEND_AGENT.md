...
- `MarketService` refreshes quotes, metrics, history, options-derived fields, and market status.
- `ScreenerService` maps allowed filter fields through `COLUMN_MAP`; unsupported fields should raise clear validation errors.
- `WatchlistService` and `PortfolioService` currently create missing symbols with NASDAQ defaults.
- `NewsService` uses RSS feeds and VADER sentiment; earnings calendar has fallback behavior.
- Scheduler jobs are in `scheduler_service.py`, all using `max_instances=1` and coalescing.
