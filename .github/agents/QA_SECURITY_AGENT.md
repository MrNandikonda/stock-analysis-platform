...
Use this with `.github/instructions/qa-security.instructions.md`.
...
- Frontend test setup only when `package.json` supports the test command and dependencies.
...
- Do not claim frontend tests are runnable unless `frontend/package.json` actually supports them.
...
- Frontend test files exist, but manifest support must be verified before running or documenting frontend tests.
